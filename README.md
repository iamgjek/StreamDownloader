...
- [https://stream.i-design.app/](https://stream.i-design.app/)
...
- **API**：`http://127.0.0.1:8000`
- **資料庫**：本機開發預設使用 `backend/users.db`（SQLite）；需要更換資料庫時可透過 `DATABASE_URL` 環境變數設定
...
| 元件 | 平台 | 自動部署 |
|------|------|----------|
| 前端 | Vercel（`frontend/`） | 在 Vercel 連結 GitHub repo 後，push 即自動部署 |
| 後端 | AWS EC2 | push 到 `main` 且變更 `backend/**` 時，由 GitHub Actions 自動部署 |

後端 CI/CD 與 EC2 一次性設定請見 [`deploy/aws-ec2-rds.md`](deploy/aws-ec2-rds.md) 第 10 節。
